...
| ImageNet backbones, frozen ViT | at chance — **but with RANDOM weights, not ImageNet ones**. torchvision's weight fetch is blocked in this environment (`download.pytorch.org` 403, still true; huggingface.co also unreachable), so these are random-projection baselines and **ImageNet transfer is untested, not closed**. What they do establish is the size story: 11.2 M / 66.8 M / 85.8 M all at chance with no ordering by capacity | `frozen_vit.md`, `pretrained_backbones.md`, `big_backbones.md` |
