...
| ImageNet backbones, frozen ViT | **settled, with real ImageNet weights.** `frozen_vit.md` is the proper test: checkpoint supplied manually (152 tensors, 0 missing/0 unexpected keys), backbone frozen at 85.8 M with a 2,307-param head — **macroP 0.501 / precET 0.332**, re-confirmed post-fix to three decimals (its spectrogram input was untouched by the fixes), against the current reported 0.652 / 0.654 — a gap of −0.151 / −0.322. Above chance (0.501 vs 0.33), so ImageNet features carry *some* signal, but 85.8 M pretrained parameters underperform ten numbers off a Welch spectrum. The *earlier* resnet/wide-resnet/ViT rows used RANDOM weights (`download.pytorch.org` blocked, still is) and are random-projection baselines — do not cite those as the transfer-learning test | `frozen_vit.md`, `pretrained_backbones.md`, `big_backbones.md` |
...
| **NewData epoch selection** (tremor-selected vs steady-selected vs none) | **all three tie** — macroP 0.646 / 0.648 / 0.648, null on every column, across a **6x range in in-band purity** (0.118 / 0.571 / 0.697). The pipeline band-limits to 3-15 Hz first, so selection was redundant with it. **Drop it**: `segment=False` removes the project's only cohort-specific preprocessing step and the only tremor-correlated selection criterion, at zero cost | `newdata_epoch_choice.md` |
